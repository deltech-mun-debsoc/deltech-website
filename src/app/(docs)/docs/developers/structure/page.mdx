...
- **Something the box needs?** `deploy/`. It is synced to `/srv/mun` on every deploy.

Do not create a shared abstraction for a single caller. Leave code beside its only user until a
second one exists.
