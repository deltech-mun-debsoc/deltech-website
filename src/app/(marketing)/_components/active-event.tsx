...
export function ActiveEvent({ content }: { content: Content }) {
  ...
  return <section className="relative overflow-hidden bg-foreground text-background">
    ...
    <div className="section-shell relative grid gap-12 py-24 lg:grid-cols-[1fr_0.62fr] lg:items-end sm:py-32">
      <div><p className="font-mono text-sm font-bold uppercase tracking-[0.18em] text-gold-300">{content.activeEventLabel || kind}</p><h1 className="mt-7 max-w-[10ch] font-heading text-6xl leading-[0.88] sm:text-8xl">{content.activeEventName}</h1><p className="mt-7 max-w-xl text-xl leading-relaxed text-background/70">{eventBrief}</p></div>
