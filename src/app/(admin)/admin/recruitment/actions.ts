...
// Assigning someone to a cycle is what grants recruitment authority. It says
// nothing about their dashboard access, which stays governed by User.role.
export async function assignRecruitmentMember(input: {
  cycleId: string
  email: string
  ...
  if (!["JC", "MAINTAINER", "ADMIN"].includes(input.role)) {
    return { ok: false, error: "Invalid recruitment role." }
  }

  try {
    const ctx = await requireRecruitmentAction(input.cycleId, "cycle.assignStaff")

    const user = await prisma.user.findUnique({ where: { email }, select: { id: true, role: true } })
    if (!user) {
      return {
        ok: false,
        error: "No account with that email. Invite them from Users first, then assign them here.",
      }
    }

    await prisma.$transaction(async (tx) => {
      const existing = await tx.recruitmentMember.findUnique({
        where: { cycleId_userId: { cycleId: input.cycleId, userId: user.id } },
        select: { id: true, role: true, isActive: true },
      })

      await tx.recruitmentMember.upsert({
        where: { cycleId_userId: { cycleId: input.cycleId, userId: user.id } },
        create: {
          cycleId: input.cycleId,
          userId: user.id,
        ...
        // Re-assigning reactivates rather than creating a rival row, so the
        // per-cycle unique constraint always holds.
        update: { role: input.role, isActive: true, revokedAt: null, revokedById: null },
      })

      await auditRecruitmentTx(tx, {
        eventType: "member.assign",
        actor: { id: ctx.userId, email: ctx.email, role: ctx.role },
        cycleId: input.cycleId,
        previousState: existing ? { role: existing.role, isActive: existing.isActive } : null,
        newState: { role: input.role, isActive: true },
        meta: { targetEmail: email, targetAppRole: user.role, implicit: ctx.implicit },
      })
    })
...
// Revoking deactivates the membership; it never deletes it, because evaluations
// and staff assignments reference it and history must survive.
export async function revokeRecruitmentMember(input: {
  cycleId: string
  memberId: string
}): Promise<ActionResult> {
  try {
    ...
    const member = await prisma.recruitmentMember.findFirst({
      where: { id: input.memberId, cycleId: input.cycleId },
      select: { id: true, role: true, isActive: true, userId: true, user: { select: { email: true } } },
    })
    if (!member) return { ok: false, error: "That member is not on this cycle." }
    if (!member.isActive) return { ok: true } // idempotent

    // Don't let an admin strip the last admin off a live cycle and lock everyone out.
    if (member.role === "ADMIN") {
      const admins = await prisma.recruitmentMember.count({
        where: { cycleId: input.cycleId, role: "ADMIN", isActive: true },
      })
      if (admins <= 1) {
        return { ok: false, error: "This is the cycle's last administrator. Assign another one first." }
      }
    }

    await prisma.$transaction(async (tx) => {
      await tx.recruitmentMember.update({
        where: { id: member.id },
        data: { isActive: false, revokedAt: new Date(), revokedById: ctx.userId },
      })
      await auditRecruitmentTx(tx, {
        eventType: "member.revoke",
        actor: { id: ctx.userId, email: ctx.email, role: ctx.role },
        cycleId: input.cycleId,
        previousState: { role: member.role, isActive: true },
        newState: { isActive: false },
        meta: { targetEmail: member.user.email, implicit: ctx.implicit },
      })
    })
