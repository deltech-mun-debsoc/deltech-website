...
export async function POST(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (!contactId) {
    return oneClick
      ...
      : NextResponse.redirect(new URL(`/unsubscribe/${encodeURIComponent(token)}`, req.url), 303)
  }
  ...
  return oneClick
    ...
    : NextResponse.redirect(new URL(`/unsubscribe/${encodeURIComponent(token)}?done=1`, req.url), 303)
}
