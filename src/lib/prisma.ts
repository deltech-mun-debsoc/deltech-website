...
const configuredPoolMax = Number(process.env.DATABASE_POOL_MAX ?? "25");
const pool = new Pool({
  ...
  // the Neon pooled endpoint is designed to multiplex them. Connections are
  // opened lazily and retired quickly, so ordinary low traffic stays small.
  max: Number.isInteger(configuredPoolMax) && configuredPoolMax > 0 ? configuredPoolMax : 25,
